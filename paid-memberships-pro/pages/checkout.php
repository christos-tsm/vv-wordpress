						...
						if ($pmpro_show_cvv) { ?>
							<div class="<?php echo esc_attr(pmpro_get_element_class('pmpro_checkout-field pmpro_payment-cvv', 'pmpro_payment-cvv')); ?>">
								...
								<input id="CVV" name="CVV" type="text" size="4" value="<?php if (!empty($_REQUEST['CVV'])) {
																						...
																						} ?>" class="<?php echo esc_attr(pmpro_get_element_class('input', 'CVV')); ?>" />
							</div>
						<?php } ?>
						<?php if ($pmpro_show_discount_code) { ?>
			...
			<?php } else { ?>
				...
				?>
					<span id="pmpro_submit_span">
						...
						<input type="submit" id="pmpro_btn-submit" class="<?php echo esc_attr(pmpro_get_element_class('pmpro_btn pmpro_btn-submit-checkout', 'pmpro_btn-submit-checkout')); ?>" value="<?php if ($pmpro_requirebilling) {
																																																			esc_html_e('Submit and Check Out', 'paid-memberships-pro');
																																																		} else {
