...
<form id="custom-restaurant-form" class="form form--fluid" method="post" enctype="multipart/form-data" action="<?php echo esc_url(get_permalink()); ?>">
    <?php if ($edit_mode) :  ?>
        <input type="hidden" name="post_id" value="<?php echo $post_id; ?>">
        <input type="hidden" name="edit_mode" value="1">
    <?php endif; ?>
    <input type="hidden" name="uuid" value="<?= $current_user->ID ?>">
    <input type="hidden" name="post-type" value="restaurants">
    ...
    <div class="form-row form-row--col">
        <input type="hidden" name="taxonomy" value="restaurant-categories">
        <label for="cat"><?php _e('Κατηγορίες'); ?></label>
        <?php
        if ($post_id && $edit_mode) :
            $selected_category = ''; // initialize variable to hold the selected category
            $categories = get_the_terms($post_id, 'restaurant-categories'); // get the categories for the post
    ...
    <div class="form-row form-row--col">
        <div class="acf-label">
            ...
            <p class="description"><?php _e('Στην διεύθυνση <a href="https://maps.google.com" target="_blank" rel="noopener">maps.google.com</a>, βάλτε την διεύθυνσή σας και πατήστε δεξί click στο location pin που σας εμφανίζει. Οι τιμές σε μορφή "23,502934, 23,5035525" είναι οι συντεταγμένες της επιχείρησής σας. Αριστερό κλικ σε αυτές για αντιγραφή και έπειτα κάντε επικόλληση εδώ.') ?></p>
        </div>
