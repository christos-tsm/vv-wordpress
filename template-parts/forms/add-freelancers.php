...
<div class="store-form__header">
    <?php if ($post_id) : ?>
        <h2 class="subsection-title"><?php _e('Επεξεργαστείτε τα στοιχεία σας'); ?></h2>
    <?php else : ?>
        <h3 class="section-subtitle section-subtitle--dashboard"><?php _e('Βήμα 2ο') ?></h3>
        <h2 class="subsection-title"><?php _e('Συμπληρώστε τα στοιχεία σας'); ?></h2>
    <?php endif; ?>
</div>
<form id="custom-freelancer-form" class="form form--fluid" method="post" enctype="multipart/form-data" action="<?php echo esc_url(get_permalink()); ?>">
    ...
    <div class="form-row form-row--col">
        <label for="municipality"><?php _e('Περιοχή'); ?></label>
