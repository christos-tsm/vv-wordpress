...
<form id="custom-shop-form" class="form form--fluid" method="post" enctype="multipart/form-data" action="<?php echo esc_url(get_permalink()); ?>">
    <?php if ($edit_mode) :  ?>
        <input type="hidden" name="post_id" value="<?php echo $post_id; ?>">
        <input type="hidden" name="edit_mode" value="1">
    <?php endif; ?>
    <input type="hidden" name="uuid" value="<?= $current_user->ID ?>">
    <input type="hidden" name="post-type" value="shops">
    ...
    <div class="form-row form-row--col">
        <input type="hidden" name="taxonomy" value="shop-categories">
        ...
        if ($post_id && $edit_mode) :
            ...
            $categories = get_the_terms($post_id, 'shop-categories'); // get the categories for the post
            if ($categories && !is_wp_error($categories)) {
                $category_ids = wp_list_pluck($categories, 'term_id'); // get the IDs of the categories
                $args = array(
                    'taxonomy' => 'shop-categories',
                    'orderby' => 'name',
                    'hide_empty' => false,
